...
function hasMeaningfulFilters(
    ...
    for (const [k, v] of Object.entries(sp)) {
        ...
        if (
            Array.isArray(v) ? v.join("").trim() : (v as string | undefined)?.trim()
        ) {
            return true;
        }
    }
...
async function runBasicFallback(
    ...
    const url = `${API_BASE_URL}/search/basic${
...
async function fetchPerspectives(): Promise<Option[]> {
    const res = await fetch(`${API_BASE_URL}/perspectives/`, {
        cache: "no-store",
    });
    ...
    return (
        <div>
                ...
                >
                    ...
                    <div>
                        <MultiSelectDropdown
                            ...
                            defaultSelectedIds={
                                collectionDefaultId ? [collectionDefaultId] : []
                            }
                            multiple={false}
                    ...
                    <div style={{ display: "grid", gap: 6, maxWidth: 360 }}>
                        ...
                        <select
                            name="include_manual"
                            defaultValue={get(sp, "include_manual")}
                            style={selectStyle}
                        >
            ...
            {usedFallback ? (
                ...
                >
                    Advanced search failed on the API, so we used a basic-search fallback
                    for this query.
                </div>
            ) : null}
            ...
            {meaningful && (results.length > 0 || !error) ? (
                <div style={{ marginBottom: 8 }}>
                    <Link
                        href={hrefWith(sp, { show_filters: "1" })}
